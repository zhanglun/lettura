...
Output the summary directly, no prefix, no quotes."#,
    ...
    let result = llm.complete(&prompt, "You are a precise content analyst.").await?;
...
mod tests {
    ...
    async fn test_generate_summary_with_mock() {
        let mock = crate::ai::llm::MockLLM::new("OpenAI released GPT-4o mini with improved performance.");
