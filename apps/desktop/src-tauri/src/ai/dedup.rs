...
pub fn persist_dedup_results(
    ...
    for group in groups {
        ...
        let density = evaluate_information_density_rule_based(
            &"x".repeat(content_lengths[group.primary_index]),
        );

        diesel::update(
            article_ai_analysis::table.filter(article_ai_analysis::id.eq(primary_aid)),
        )
        ...
        for &dup_idx in &group.duplicate_indices {
            ...
            diesel::update(
                article_ai_analysis::table
                    .filter(article_ai_analysis::article_id.eq(dup_article_id)),
            )
            .set((
