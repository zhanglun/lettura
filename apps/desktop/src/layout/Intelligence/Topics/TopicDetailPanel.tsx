...
export function TopicDetailPanel({
	...
	const openArticle = (articleId: number) => {
		const feedUuid = feedUuidMap.get(articleId);
		if (feedUuid) {
			navigate(
				RouteConfig.LOCAL_ARTICLE.replace(":uuid", feedUuid).replace(
					":id",
					String(articleId),
				),
			);
	...
	const openBestEvidence = () => {
		const article = [...topic.articles].sort(
			(a, b) => b.relevance_score - a.relevance_score,
		)[0];
		if (article && openArticle(article.article_id)) {
			return;
		}
		const sourcesSection = document.getElementById("topic-detail-sources");
		if (typeof sourcesSection?.scrollIntoView === "function") {
	...
	if (loading) {
		return (
			<aside className="flex flex-col items-center justify-center h-full border-l border-[var(--gray-4)] bg-[var(--color-background)]">
				<Layers size={36} className="mb-3 text-[var(--gray-8)] animate-pulse" />
	...
	return (
		<aside className="topic-detail-panel">
				...
				<div className="flex items-center gap-2 mb-2">
					<span
						className={cn(
							"topic-status-tag",
							topic.is_following
								? "topic-status-tag--following"
								: topic.is_muted
								? "topic-status-tag--muted"
								: "topic-status-tag--discovered",
						)}
					>
						{topic.is_following
			...
			<div className="topic-detail-scroll">
				{(topic.topic_summary ||
				...
				{topic.articles.length === 0 ? (
					...
					<section id="topic-detail-sources" className="topic-detail-card">
						<div className="topic-detail-card-title">
								...
								.map((article, idx, list) => {
									...
									return (
										<button
											...
											className={cn(
												"topic-start-card",
												feedUuid ? "cursor-pointer" : "",
											)}
											onClick={() => {
												openArticle(article.article_id);
											}}
										>
