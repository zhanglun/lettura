...
export const createTopicSlice: StateCreator<TopicSlice> = (set, get) => ({
	...
	fetchTopics: async (status, sort) => {
		...
		try {
			...
			set({
				...
				loading: false,
				followingTopicIds: new Set(
					topics.filter((t) => t.is_following).map((t) => t.id),
				),
				mutedTopicIds: new Set(
					topics.filter((t) => t.is_muted).map((t) => t.id),
				),
			});
		} catch (e) {
	...
	fetchTopicDetail: async (topicUuid) => {
		const requestId = get().detailRequestId + 1;
		set({ detailLoading: true, error: null, detailRequestId: requestId });
		try {
			const detail: TopicDetail = await invoke("get_topic_detail", {
				topicId: String(topicUuid),
			});
			if (get().detailRequestId === requestId) {
	...
	selectTopic: (uuidOrId: string | number) => {
		const topic = get().topics.find(
			(t) => t.uuid === uuidOrId || t.id === Number(uuidOrId),
		);
		if (topic) {
	...
	followTopic: async (topicId) => {
		...
		set({
			...
			topics: topics.map((t) =>
				t.id === topicId ? { ...t, is_following: true } : t,
			),
			selectedTopic:
	...
	unfollowTopic: async (topicId) => {
		...
		set({
			...
			topics: topics.map((t) =>
				t.id === topicId ? { ...t, is_following: false, is_muted: false } : t,
			),
			selectedTopic:
				selectedTopic && selectedTopic.id === topicId
	...
	muteTopic: async (topicId) => {
		...
		set({
			...
			followingTopicIds: newFollowing,
			topics: topics.map((t) =>
				t.id === topicId ? { ...t, is_following: false, is_muted: true } : t,
			),
			selectedTopic:
				selectedTopic && selectedTopic.id === topicId
	...
	unmuteTopic: async (topicId) => {
		...
		set({
			...
			followingTopicIds: newFollowing,
			topics: topics.map((t) =>
				t.id === topicId ? { ...t, is_following: true, is_muted: false } : t,
			),
			selectedTopic:
				selectedTopic && selectedTopic.id === topicId
